...
class WordleFeedbackGUI:
    def __init__(self, root):
        ...
        # Bind click event for selecting a word from the list
        self.top_words_listbox.bind("<ButtonRelease-1>", self.select_word_from_list)
    ...
    def select_word_from_list(self, event):
        """Select a word from the top words list and update the guess."""
        selected_index = self.top_words_listbox.curselection()  # Get the selected item index
        if selected_index:
            selected_word = self.top_words_listbox.get(selected_index)  # Get the selected word
            self.update_guess(selected_word)  # Update the displayed guess with the selected word
            print(f"Selected guess from list: {selected_word}")  # Debug statement to confirm selection
